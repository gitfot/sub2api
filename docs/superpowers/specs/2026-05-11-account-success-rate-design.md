...
- 未传 `account_id` 时，返回“全部正常账号”的汇总趋势，保持现有默认展示。
- 传入 `account_id` 时，只返回该账号的趋势数据。
- `account_id` 只接受当前正常账号，即 `status=active && schedulable=true`。
- 传入不存在、非正常、或已删除账号时返回空数据，不回退到“全部账号”。
