...
SELECT
  ...
  COUNT(*) FILTER (WHERE ` + opsProviderUpstreamErrorPredicateSQL("") + ` AND NOT is_business_limited AND ` + opsUpstreamEffectiveStatusSQL("") + ` NOT IN (429, 529)) AS upstream_excl,
  COUNT(*) FILTER (WHERE ` + opsProviderUpstreamErrorPredicateSQL("") + ` AND NOT is_business_limited AND ` + opsUpstreamEffectiveStatusSQL("") + ` = 429) AS upstream_429,
  COUNT(*) FILTER (WHERE ` + opsProviderUpstreamErrorPredicateSQL("") + ` AND NOT is_business_limited AND ` + opsUpstreamEffectiveStatusSQL("") + ` = 529) AS upstream_529
FROM ops_error_logs
` + where + `
GROUP BY 1
ORDER BY 1 ASC`
